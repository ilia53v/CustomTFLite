...
class YoloV8Segmentor(private val context: Context) {
   ...
   /* fun runInference(bitmap: Bitmap, confidenceThreshold: Float): Triple<List<List<List<PointF>>>, FloatArray?, List<YoloObject>> {
        ...
        for (i in 0 until 8400) {
            ...
            val minAcceptableBoxSize = 0.01f // например, 1% от изображения
            val isSmallObject = (w * h) < minAcceptableBoxSize
            if (conf <= confidenceThreshold && !isSmallObject) continue
    ...
    fun runInference(bitmap: Bitmap, confidenceThreshold: Float): Triple<List<List<List<PointF>>>, FloatArray?, List<YoloObject>> {
        ...
        val confidenceThresholdWeak = 0.15f
